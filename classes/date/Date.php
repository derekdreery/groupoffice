...
// +----------------------------------------------------------------------+
//
// $Id: Date.php,v 1.4 2005/10/21 13:13:02 mschering Exp $

/**@#+
 * Include supporting classes
 */
require_once $GO_CONFIG->class_path.'date/Date/TimeZone.php';
require_once $GO_CONFIG->class_path.'date/Date/Calc.php';
require_once $GO_CONFIG->class_path.'date/Date/Span.php';
/**@#-*/
 ...
 *
 * Merijn Schering: Renamed classes because somehow they conflicted with PEAR 
 * installations
 *
...
class go_date
{
     ...
     */
    function go_date($date = null)
    {
        $this->tz = go_date_TimeZone::getDefault();
        if (is_null($date)) {
     ...
     *  <code>%E  </code>  number of days since unspecified epoch (integer, go_date_Calc::dateToDays()) <br>
    ...
    function format($format)
    {
        ...
        for($strpos = 0; $strpos < strlen($format); $strpos++) {
            ...
            if ($char == "%") {
                ...
                switch ($nextchar) {
                case "a":
                    $output .= go_date_Calc::getWeekdayAbbrname($this->day,$this->month,$this->year);
                    break;
                case "A":
                    $output .= go_date_Calc::getWeekdayFullname($this->day,$this->month,$this->year);
                    break;
                case "b":
                    $output .= go_date_Calc::getMonthAbbrname($this->month);
                    break;
                case "B":
                    $output .= go_date_Calc::getMonthFullname($this->month);
                    break;
                case "C":
                ...
                case "e":
                    ...
                    break;
                case "E":
                    $output .= go_date_Calc::dateToDays($this->day,$this->month,$this->year);
                    break;
                case "H":
                ...
                case "I":
                    ...
                    break;
                case "j":
                    $output .= go_date_Calc::julianDate($this->day,$this->month,$this->year);
                    break;
                case "m":
                ...
                case "T":
                    ...
                    break;
                case "w":
                    $output .= go_date_Calc::dayOfWeek($this->day,$this->month,$this->year);
                    break;
                case "U":
                    $output .= go_date_Calc::weekOfYear($this->day,$this->month,$this->year);
                    break;
                case "y":
     ...
     * go_date_TimeZone object.  Does not alter the date/time,
     * only assigns a new time zone.  For conversion, use
     * convertTZ().
     *
     * @access public
     * @param object go_date_TimeZone $tz the go_date_TimeZone object to use, if called 
     * with a paramater that is not a go_date_TimeZone object, will fall through to
     * setTZbyID(). 
     */
    function setTZ($tz)
    {
    ...
    {
        if (go_date_TimeZone::isValidID($id)) {
            $this->tz = new go_date_TimeZone($id);
        } else {
            $this->tz = go_date_TimeZone::getDefault();
        }
    }
     ...
     * this date in this date's time zone.  See go_date_TimeZone::inDaylightTime()
    ...
    function toUTC()
    {
        ...
        $this->tz = new go_date_TimeZone('UTC');
    }
     ...
     *
     * @access public
     * @param object go_date_TimeZone $tz the Date::TimeZone object for the conversion time zone
     */
    ...
    {
       if (go_date_TimeZone::isValidID($id)) {
          $tz = new go_date_TimeZone($id);
       } else {
          $tz = go_date_TimeZone::getDefault();
       }
       $this->convertTZ($tz);
    }
    ...
    {
        $this->addSpan(new go_date_Span((integer)$sec));
    }
     ...
     *
     * @access public
     * @param object go_date_Span $span the time span to add
     */
    function addSpan($span)
    {
        ...
        if ($this->minute >= 60) {
            ...
            if ($this->hour >= 24) {
                list($this->year, $this->month, $this->day) =
                    sscanf(go_date_Calc::nextDay($this->day, $this->month, $this->year), "%04s%02s%02s");
                $this->hour -= 24;
            }
            $this->minute -= 60;
        }
        ...
        $d = go_date_Calc::dateToDays($this->day, $this->month, $this->year);
        $d += $span->day;

        list($this->year, $this->month, $this->day) =
            sscanf(go_date_Calc::daysToDate($d), "%04s%02s%02s");
    ...
    {
        $this->subtractSpan(new go_date_Span($sec));
    }
     ...
     *
     * @access public
     * @param object go_date_Span $span the time span to subtract
     */
    function subtractSpan($span)
    {
        ...
        if ($this->minute < 0) {
            ...
            if ($this->hour < 0) {
                list($this->year, $this->month, $this->day) =
                    sscanf(go_date_Calc::prevDay($this->day, $this->month, $this->year), "%04s%02s%02s");
                $this->hour += 24;
            }
            $this->minute += 60;
        }
    ...
    {
        $d1->convertTZ(new go_date_TimeZone('UTC'));
        $d2->convertTZ(new go_date_TimeZone('UTC'));
        $days1 = go_date_Calc::dateToDays($d1->day, $d1->month, $d1->year);
        $days2 = go_date_Calc::dateToDays($d2->day, $d2->month, $d2->year);
    ...
    function isLeapYear()
    {
        return go_date_Calc::isLeapYear($this->year);
    }
    ...
    {
        return go_date_Calc::julianDate($this->day, $this->month, $this->year);
    }
    ...
    {
        return go_date_Calc::dayOfWeek($this->day, $this->month, $this->year);
    }
    ...
    {
        return go_date_Calc::weekOfYear($this->day, $this->month, $this->year);
    }
    ...
    {
        return go_date_Calc::quarterOfYear($this->day, $this->month, $this->year);
    }
    ...
    {
        return go_date_Calc::daysInMonth($this->month, $this->year);
    }
    ...
    {
        return go_date_Calc::weeksInMonth($this->month, $this->year);
    }
    ...
    {
        if ($abbr) {
            return go_date_Calc::getWeekdayAbbrname($this->day, $this->month, $this->year, $length);
        } else {
            return go_date_Calc::getWeekdayFullname($this->day, $this->month, $this->year);
        }
    }
    ...
    {
        if ($abbr) {
            return go_date_Calc::getMonthAbbrname($this->month);
        } else {
            return go_date_Calc::getMonthFullname($this->month);
        }
    }
    ...
    function getNextDay()
    {
        $day = go_date_Calc::nextDay($this->day, $this->month, $this->year, "%Y-%m-%d");
    ...
    function getPrevDay()
    {
        $day = go_date_Calc::prevDay($this->day, $this->month, $this->year, "%Y-%m-%d");
        $date = sprintf("%s %02d:%02d:%02d", $day, $this->hour, $this->minute, $this->second);
        $newDate = new Date();
        $newDate->setDate($date);
        return $newDate;
    ...
    {
        $day = go_date_Calc::nextWeekday($this->day, $this->month, $this->year, "%Y-%m-%d");
        $date = sprintf("%s %02d:%02d:%02d", $day, $this->hour, $this->minute, $this->second);
        $newDate = new Date();
        $newDate->setDate($date);
        return $newDate;
    ...
    {
        $day = go_date_Calc::prevWeekday($this->day, $this->month, $this->year, "%Y-%m-%d");
        $date = sprintf("%s %02d:%02d:%02d", $day, $this->hour, $this->minute, $this->second);
        $newDate = new Date();
        $newDate->setDate($date);
        return $newDate;
