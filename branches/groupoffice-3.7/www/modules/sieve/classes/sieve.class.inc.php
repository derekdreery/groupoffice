...
$include_path = $GO_CONFIG->class_path . 'pear/' . PATH_SEPARATOR;
...
class sieve {
	...
	private function rewrite_host($host) {
		...
		if (isset($GO_CONFIG->sieve_rewrite_hosts)) {

			$maps = explode(',', $GO_CONFIG->sieve_rewrite_hosts);
	...
	public function connect($username, $password='', $host='localhost', $port=2000, $auth_type=null, $usetls=true, $disabled=array(), $debug=false) {
		...
		go_debug("sieve::connect($username, ***, $host, $port, $auth_type, $usetls, $disabled)");
	...
	public function save($name = null) {
		...
		if (PEAR::isError($this->sieve->installScript($name, $script, true)))
