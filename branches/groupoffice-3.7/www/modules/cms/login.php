	...
	try{
		...
		if (!$GO_AUTH->login($username, $password))
		{
			...
			if (isset($_POST['remind']))
			{
				...
				SetCookie("GO_UN",$c->encrypt($username),time()+3600*24*30,"/",'',!empty($_SERVER['HTTPS']),false, true);
				SetCookie("GO_PW",$c->encrypt($password),time()+3600*24*30,"/",'',!empty($_SERVER['HTTPS']),false, true);
			}
