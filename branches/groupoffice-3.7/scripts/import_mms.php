...
function get_calendar($username){
	...
	$calendar['acl_id']=$GO_SECURITY->get_new_acl('calendar');
...
function get_category($category_name){
	...
	$category['user_id']=1;
	$category['acl_id']=$GO_SECURITY->get_new_acl('notes', $category['user_id']);
...
require_once ($GO_MODULES->modules['tickets']['class_path'] . "tickets.class.inc.php");
$tickets = new tickets();
...
function get_type($name){
	...
	$type ['name']=$name;
	$type ['user_id']=1;
	$type ['acl_id']=$GO_SECURITY->get_new_acl('type', $type['user_id']);
...
$existing_ticket=false;
			if(!$existing_ticket){
				...
				$message['is_note'] = 0;
				$message['user_id'] = $GO_SECURITY->user_id;
