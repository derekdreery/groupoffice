...
$response['feedback']='An unknown error occurred';
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	{
		$response['feedback'] = $lang['servermanager']['invalidHost'];
	}elseif(file_exists('/var/lib/mysql/'.$new_trial['name']) ||
	file_exists('/etc/apache2/sites-enabled/'.$new_trial['name']) ||
	$sm->get_new_trial_by_name(($new_trial['name']))
	)
	{
		$response['feedback'] = $lang['servermanager']['duplicateHost'];
	}else
	{
		...
		$response['feedback']='<h1>Thank you</h1>
		<p>An e-mail has been sent to '.$new_trial['email'].'. Open it and follow the instructions to complete your trial.</p>';
