...
$path = $GLOBALS['GO_CONFIG']->file_storage_path.$files->build_path($folder);
...
while($file = array_shift($_FILES))
{
	...
	if (is_uploaded_file($file['tmp_name']))
	{
		...
		{				
			...
			if($_SESSION['GO_SESSION']['chunked_upload_size']>$GLOBALS['GO_CONFIG']->max_file_size)
			{
			...
			//	go_debug('Uploaded file too big: '.$_SESSION['GO_SESSION']['chunked_upload_size'].' -> '.$GLOBALS['GO_CONFIG']->max_file_size);
			...
			if(!empty($_POST['jufinal']))
			{
				...
				if($GLOBALS['GO_MODULES']->has_module('workflow'))
				{
