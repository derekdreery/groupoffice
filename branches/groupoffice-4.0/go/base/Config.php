 ...
 * @version $Id: config.class.inc.php 7678 2011-06-23 08:22:44Z mschering $
 ...
 */
	...
	function __construct() {
		...
		//if(isset($this->session_config_file))
			//GO::debug('Used config file from session', $this);
	...
	public function set_default_session(){
		...
		if(!isset(GO::session()->security_token))
		{
			GO::session()->decimal_separator = $this->default_decimal_separator;
			GO::session()->thousands_separator = $this->default_thousands_separator;
			GO::session()->date_separator = $this->default_date_separator;
			GO::session()->date_format = Date::get_dateformat( $this->default_date_format, GO::session()->date_separator);
			GO::session()->time_format = $this->default_time_format;
			GO::session()->currency = $this->default_currency;
			GO::session()->timezone = $this->default_timezone;
			GO::session()->country = $this->default_country;
			GO::session()->sort_name = 'last_name';
			GO::session()->auth_token=String::random_password('a-z,1-9', '', 30);
			//some url's require this token to be appended
			GO::session()->security_token=String::random_password('a-z,1-9', '', 10);

			GO::debug('Setup new session '.GO::session()->security_token);
		}
	...
	function __destruct() {
		if($this->debug_log) {
			...
			GO::debug('Page load took: '.(getmicrotime()-$this->loadstart).'ms', $this);
	...
	function get_config_file() {
		...
		if(basename($_SERVER['PHP_SELF'])=='index.php'){
			unset(GO::session()->config_file);
		}

		if(isset(GO::session()->config_file)) {
			$this->session_config_file=true;
			return GO::session()->config_file;
		}else {
			$config_dir = $this->root_path;
			$config_file = $config_dir.'config.php';
			if(@file_exists($config_file)) {
				GO::session()->config_file=$config_file;
				return $config_file;
			}
			...
			while(!isset(GO::session()->config_file)){
				$count++;
				$config_file = $config_dir.'config.php';
	...
	function set_full_url() {
		...
		if(isset($_SERVER["HTTP_HOST"])) {
			if(!isset(GO::session()->full_url) && isset($_SERVER["HTTP_HOST"])) {
				$https = (isset($_SERVER["HTTPS"]) && ($_SERVER["HTTPS"] == "on" || $_SERVER["HTTPS"] == "1")) || !empty($_SERVER["HTTP_X_SSL_REQUEST"]);
				GO::session()->full_url = 'http';
				if ($https) {
					GO::session()->full_url .= "s";
				}
				/*$url .= "://";
				...
				GO::session()->full_url .= '://'.$_SERVER["HTTP_HOST"].$this->host;
			}
			$this->full_url=GO::session()->full_url;
		}else
		{
			GO::session()->full_url=$this->full_url;
		}
