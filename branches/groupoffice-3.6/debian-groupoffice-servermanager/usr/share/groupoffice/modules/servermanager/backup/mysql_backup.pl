...
sub copyDB {
  ...
  foreach my $line (@dblist) {
    ...
    if (($sqluser ne '') and ($sqlpass ne '')){
      ...
      system('/usr/bin/mysqlhotcopy --allowold --user=' . $sqluser . ' --password=' . $sqlpass . ' ' . $db . ' ' . $DATADIR . ' 2>&1>/dev/null');
    }
    else {
      # System call to copy the database.  Overwrite existing directories.
      system('/usr/bin/mysqlhotcopy --allowold ' . $db . ' ' . $DATADIR .' 2>&1>/dev/null');
    }
    compressDB($db);
  }
}
...
sub compressDB {
  ...
  foreach my $line (@dblist){
    ...
    if ( -d $DATADIR . '/' . $db) {
      ...
      system('cd ' . $DATADIR . ';/bin/tar cvzf ' . $db . '.tgz ' . $db .' 2>&1>/dev/null');
