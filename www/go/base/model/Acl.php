...
class Acl extends \GO\Base\Db\ActiveRecord {
	...
	public static function getUserPermissionLevel($aclId, $userId=false, $checkGroupPermissionOnly=false) {
		...
		$model = \GO\Base\Model\AclUsersGroups::model()->findSingle($findParams);
		if($model)
	...
	public function addUser($userId, $level=Acl::READ_PERMISSION) {
		...
		{		
			...
			$usersGroup = new \GO\Base\Model\AclUsersGroups();
	...
	public function hasGroup($groupId){
		return \GO\Base\Model\AclUsersGroups::model()->findByPk(array(
				'acl_id'=>$this->id,
				'group_id'=>$groupId,
				'user_id'=>0
						));
	...
	public function getUsers(){
		...
		return \GO\Base\Model\User::model()->find($findParams);
	}
	...
	public function getGroups(){
		...
		return \GO\Base\Model\Group::model()->find($findParams);
	}
	 ...
	 * @return Array of \GO\Base\Model\User 
	 */
...
//		return $stmt;
		...
		$joinCriteria = \GO\Base\Db\FindCriteria::newInstance()
										->addModel(\GO\Base\Model\AclUsersGroups::model(), 'a')
										->addModel(\GO\Base\Model\User::model(), 't')
		...
		$stmt =  \GO\Base\Model\User::model()->find(\GO\Base\Db\FindParams::newInstance()		
						->ignoreAcl()
						->join(\GO\Base\Model\AclUsersGroups::model()->tableName(),$joinCriteria
										,'a')
						);
		...
		$stmt =  \GO\Base\Model\User::model()->find(\GO\Base\Db\FindParams::newInstance()				
						->ignoreAcl()
						->join(\GO\Base\Model\UserGroup::model()->tableName(),  \GO\Base\Db\FindCriteria::newInstance()		
...
//where acl_id=19260 and level>1
		...
		$model = \GO\Base\Model\AclUsersGroups::model()->find($findParams);
	...
	public function clear(){
		...
		if($this->description!='readonly'){
			if (!$adminUserRecordExists) {
				$aclRecord = new \GO\Base\Model\AclUsersGroups();
