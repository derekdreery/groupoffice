...
class GO_Ldapauth_Controller_Sync extends GO_Base_Controller_AbstractController{
	...
	protected function actionLookupUser($params){
		...
		$result = $ldapConn->search(GO_Ldapauth_LdapauthModule::getPeopleDn($params['uid']), $la->getUserSearchQuery($params['uid']));
	...
	protected function actionUsers($params){
		...
		$result = $ldapConn->search(GO_Ldapauth_LdapauthModule::getPeopleDn(), $la->getUserSearchQuery());
