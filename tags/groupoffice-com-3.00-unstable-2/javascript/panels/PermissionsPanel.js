 ...
 * 
 * @version $Id: PermissionsPanel.js 1857 2008-04-29 13:57:06Z mschering $
 ...
 * @extends Ext.Panel
 *
 * A panel that can be used to set permissions for a Group-Office ACL.
 * It will use an anchor layout with 100% width and 100% height automatically.
 * 
 * @constructor
...
GO.grid.PermissionsPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
		...
		this.aclGroupsStore = new GO.data.JsonStore({
			...
			baseParams: {task: "groups_in_acl", acl_id: 0},
			root: 'results',
			totalProperty: 'total',
			id: 'id',
			fields: ['id','name'],
		...
		this.aclUsersStore = new GO.data.JsonStore({
			...
			url: BaseHref+'json.php',			
			baseParams: {task: "users_in_acl", acl_id: 0},
			root: 'results',
			totalProperty: 'total',
			id: 'id',
			fields: ['id','name'],
     ...
     * 
     * @param {Number} The Group-Office acl ID.
     */
