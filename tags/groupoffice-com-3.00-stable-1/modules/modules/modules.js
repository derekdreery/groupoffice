...
GO.modules.MainPanel = function(config)
{
	...
	this.installedModulesDS = new GO.data.JsonStore({
	    ...
	    baseParams:  {task: 'installed_modules'},
	    root: 'results',
	    id:'id',
	    fields: ['name', 'description', 'id', 'sort_order', 'admin_menu','acl_read','acl_write'],
	    remoteSort: true
	});
	...
	this.availableModulesDS = new GO.data.JsonStore({
		    url: GO.settings.modules.modules.url+'json.php',
		    baseParams:  {task: 'available_modules'},
		    root: 'results',
		    id:'id',
		    fields: ['name', 'description', 'id', 'sort_order', 'admin_menu','acl_read','acl_write'],
		    remoteSort: true
  ...
  config.tbar = new Ext.Toolbar({		
			cls:'go-head-tb',
			items: [
  	//{id: 'save', iconCls: 'btn-save', text: GO.modules.lang['cmdSaveChanges'], cls: 'x-btn-text-icon', handler: this.onButtonClick, scope: this},
  	{ iconCls: 'btn-add', text: GO.modules.lang['cmdInstall'], cls: 'x-btn-text-icon', handler: this.showavailableModules, scope: this},
  	{ iconCls: 'btn-delete', text: GO.modules.lang['cmdUninstall'], cls: 'x-btn-text-icon', handler: this.uninstallModule,  scope: this},
  	{ iconCls: 'btn-permissions', text: GO.lang.strPermissions, cls: 'x-btn-text-icon', handler: this.showPermissions, scope: this}
 		] });
...
Ext.extend(GO.modules.MainPanel, GO.grid.GridPanel,{
	...
	showavailableModules : function(){
		...
		{			
			...
			this.availableModulesWin = new Ext.Window( {
				...
				items: grid,				
				buttons: [
						{text: GO.modules.lang['cmdInstall'], handler: function(){ this.installModule(grid);}, scope: this },
						{text: GO.lang['cmdClose'], handler: function(){this.availableModulesWin.hide();}, scope: this}
					]
			}); 
		}
	 ...
	 save : function()
	 {
    	...
    	{			    	
				modules[i] = {id: this.installedModulesDS.data.items[i].get('id'), sort_order: i, admin_menu: this.installedModulesDS.data.items[i].get('admin_menu')};
	   	}
			...
			conn.request({
				...
				params: {task: 'update', modules: Ext.encode(modules)},
	  ...
	  installModule : function(grid)
	  {
	   	grid.container.mask(GO.lang['waitMsgLoad']);
			...
			if (records.length > 0)
			{
				...
				conn.request({
					url: GO.settings.modules.modules.url+'action.php',
					params: {task: 'install', id: records[0].data.id},
					callback: function(options, success, response){
    ...
    uninstallModule : function()
    {
			...
			switch(this.selModel.selections.keys.length)
			{
				...
				default:
					...
					var strConfirm = t.applyTemplate({'count': this.selModel.selections.keys.length});						
				break;						
			}
    ...
    showPermissions : function()
    {
    	...
    	if(record.length > 0)
    	{
					...
					buttons:[
						{
							...
							handler: function(){this.permissionsWin.hide()}, 
							scope: this
						}
					]				
				});
			...
			this.permissionsWin.setTitle(GO.lang['strPermissions']+' ' + record[0].data.name);
    ...
    {
    	return '<div class="go-module-icon-'+record.data.id+'" style="height:16px;padding-left:22px;background-repeat:no-repeat;">'+name+'</div>';
    }	    	     	
});
