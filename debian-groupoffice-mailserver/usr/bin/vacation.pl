...
sub send_vacation_email {
   my ($email, $alias, $orig_from, $orig_to, $orig_messageid) = @_;
   my $query = qq{SELECT vacation_subject,vacation_body FROM pa_mailboxes WHERE username=?};
   my $stm = $dbh->prepare($query) or panic_prepare($query);
   $stm->execute($email) or panic_execute($query,"email='$email'");
   my $rv = $stm->rows;
   if ($rv == 1) {
...
for (@search_array) {
   ...
   if ($rv == 1) {
      ...
      send_vacation_email( $email, $addr, $from, $to, $messageid);
   }
}
