		...
		case '{friendly_single}':
			...
			<gotpl if="$authenticate_relation && $relation">
			$response['data']['permission_level']=GO::security()->has_permission(GO::security()->user_id, ${related_friendly_single}['acl_id']);
			...
			if(isset(GO::modules()->modules['files']))
			{
				...
				$full_path = GO::config()->file_storage_path.$response['data']['files_path'];
			...
			if($task=='{friendly_single}')
			{
				if(GO::modules()->has_module('customfields'))
				{
					...
					$cf = new customfields();
					$values = $cf->get_values(GO::security()->user_id, {link_type}, $response['data']['id']);				
				...
				if(GO::modules()->has_module('customfields'))
				{
					...
					$response['data']['customfields']=
						$cf->get_all_fields_with_values(
							GO::security()->user_id, {link_type}, $response['data']['id']);			
				}
		...
		case '{friendly_multiple}':
			...
			$response['permission_level']=GO::security()->has_permission(GO::security()->user_id, ${related_friendly_single}['acl_id']);
			$response['data']['write_permission']=$response['permission_level']>GO_SECURITY::READ_PERMISSION;
			if(!$response['data']['permission_level'])
			{
				throw new AccessDeniedException();
			...
			$response['total'] = ${module}->get_<gotpl if="$authenticate">authorized_</gotpl>{friendly_multiple}(<gotpl if="$authenticate">$auth_type, GO::security()->user_id, </gotpl><gotpl if="$relation">${related_field_id}, </gotpl> $query, $sort, $dir, $start, $limit);
