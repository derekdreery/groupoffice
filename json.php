...
try {
	switch($_REQUEST['task']) {
		...
		$response['total'] = $aq->get_search_queries($GO_SECURITY->user_id);
		...
		case 'email':
			...
			if(isset($GO_MODULES->modules['addressbook']) && $GO_MODULES->modules['addressbook']['read_permission']) {
				...
				if(count($addresses)<10) {
					$ab->search_company_email($GO_SECURITY->user_id, $query);
				...
				try {
					...
					foreach($delete_links as $delete_link) {
						...
						if($link[0]=='folder') {
							...
							if($GO_SECURITY->has_permission($GO_SECURITY->user_id, $record['acl_id'])<GO_SECURITY::DELETE_PERMISSION) {
								throw new AccessDeniedException();
							}
