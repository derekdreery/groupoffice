...
if (true) {
	...
	if ($book) {
		...
		$book_id = $bs->add_book(array('name' => 'Bestelbonnen', 'default_vat' => 19, 'is_purchase_orders_book' => 1, 'country' => 'NL', 'currency' => '€', 'order_id_prefix' => '%y-', 'user_id' => 1, 'acl_id' => $GO_SECURITY->get_new_acl('book')));
	}
			...
			if (isset($order['id'])) {
				...
				if ($order['status']) {
					...
					if ($status) {
						...
						$status_language['status_id'] = $order['status_id'] = $bs->add_order_status(array('book_id' => $book_id, 'acl_id' => $GO_SECURITY->get_new_acl('order_status')));
