<?php
header('Content-Type: text/html; charset=UTF-8');

require('../../Group-Office.php');

require_once($GO_MODULES->modules['cms']['class_path'].'cms.class.inc.php');
require_once($GO_MODULES->modules['cms']['class_path'].'output.class.inc.php');
require_once($GO_MODULES->modules['cms']['class_path'].'cms_smarty.class.inc.php');
$cms = new cms();

$folder = $cms->get_folder($_REQUEST['folder_id']);
$site = $cms->get_site($folder['site_id']);

function replace_template($t, $r){
	foreach($r as $key=>$v){
		if(!strpos($v, 'enclosure')){
			$v=htmlspecialchars($v,ENT_QUOTES,'UTF-8');
		}
		$t = str_replace('{'.$key.'}', $v, $t);
	}

	$t = preg_replace('/\{[^\}]*\}/', '', $t);

	return $t;
}

$t['title']=$folder['name'];
$t['link']=$GO_MODULES->modules['cms']['full_url'].'rss.php?folder_id='.$folder['id'];
$t['description']='Last messages of group '.$folder['name'];
$t['webmaster']=$t['managingEditor']=$site['webmaster'];
$t['language']=$site['language'];


$header = '<?xml version="1.0" ?>
<rss version="2.0">
<channel>
<title>{title}</title>
<link>{link}</link>
<description>{description}</description>
<language>{language}</language>
<pubDate>'.date('r').'</pubDate>
<lastBuildDate>'.date('r').'</lastBuildDate>
<docs>http://www.rssboard.org/rss-specification</docs>
<generator>'.$GO_CONFIG->product_name.' '.$GO_CONFIG->version.'</generator>
<managingEditor>{managingEditor}</managingEditor>
<webMaster>{webMaster}</webMaster>
<ttl>60</ttl>
';

$footer = '
</channel>
</rss>';

$item = '<item>
<title>{title}</title>
<link>{link}</link>
<description>{description}</description>
<pubDate>{pubDate}</pubDate>
</item>
';

echo replace_template($header, $t);

$cms->get_files($folder['id']);
while($file = $cms->next_record()){
	$ri['title']=$file['name'];
	$ri['link']=$GO_MODULES->modules['cms']['full_url'].'run.php?file_id='.$file['id'];
	$ri['pubDate']=date('r', $file['mtime']);
	$ri['description']=$file['content'];
	
	echo replace_template($item, $ri);
}
echo $footer;
