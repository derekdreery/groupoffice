...
while($GO_USERS->next_record())
{
	...
	if(!is_dir($home_dir))
	{
		mkdir($home_dir, GO::config()->folder_create_mode,true);
	}
	...
	if(empty($folder['acl_read']))
	{
		...
		$up_folder['acl_read']=GO::security()->get_new_acl('files', $GO_USERS->f('id'));
		$up_folder['acl_write']=GO::security()->get_new_acl('files', $GO_USERS->f('id'));
		$up_folder['visible']='1';
