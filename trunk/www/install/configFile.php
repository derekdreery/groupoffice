...
}else
{
	try{
		if(!empty($_POST['submitted'])){
				...
				if(!$f->exists())
					GO_Base_Html_Input::setError("file_storage_path", "File storage folder doesn't exist. Please make sure it exists.");

				GO::config()->file_storage_path=$f->path().'/';

				$f = new GO_Base_Fs_Folder($_POST['tmpdir']);
				if(!$f->exists())
					GO_Base_Html_Input::setError("tmpdir", "Temporary folder doesn't exist. Please make sure it exists.");
