...
// $Id: index.php,v 1.1 2005/04/15 21:23:30 mschering Exp $
...
if (!isset($template)) {
  $template = isset($_COOKIE['template']) ? $_COOKIE['template'] : $default_template;
}
...
if (!empty($sensor_program)) {
  if (file_exists(APP_ROOT . '/includes/mb/class.' . $sensor_program . '.inc.php')) {
    require_once('./includes/mb/class.' . $sensor_program . '.inc.php');
    $mbinfo = new mbinfo;
  } else {
...
require_once('./includes/common_functions.php'); // Set of common functions used through out the app
require_once('./includes/xml/vitals.php');
require_once('./includes/xml/network.php');
require_once('./includes/xml/hardware.php');
require_once('./includes/xml/memory.php');
require_once('./includes/xml/filesystems.php');
require_once('./includes/xml/mbinfo.php');
...
if ($template == 'xml') {
  ...
  if (PHPGROUPWARE != 1) {
    require_once('./includes/class.Template.inc.php'); // template library
  } 
  ...
  require_once('./includes/XPath.class.php');
  $XPath = new XPath();
  ...
  if (PHPGROUPWARE == 1) {
    ...
    require_once('./includes/system_footer.php');
  }
